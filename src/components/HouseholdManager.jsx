...
export default function HouseholdManager() {
  ...
  if (loading) return <p className="text-sm text-white/50">Loading household…</p>
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-2">
        <div className="flex items-center justify-between p-3">
          ...
          <span className="rounded-full bg-black/40 px-2.5 py-0.5 text-xs" style={{ color: GOLD }}>
            primary
          </span>
        </div>
        {members.map((m) => (
          <div key={m.id} className="flex items-center justify-between border-t border-white/5 p-3">
            <div>
            ...
            <button onClick={() => remove(m.id)} className="text-sm text-white/50 hover:text-red-400">
              Remove
            </button>
          </div>
        ))}
      ...
      <form onSubmit={add} className="rounded-2xl border border-white/10 bg-white/5 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <select
            ...
            className="rounded-lg border border-white/15 bg-black/30 px-3 py-2.5 text-sm outline-none focus:border-white/40"
          >
            {TYPES.map((t) => (
          ...
          <input
            ...
            inputMode="numeric"
            className="w-28 rounded-lg border border-white/15 bg-black/30 px-3 py-2.5 text-sm outline-none focus:border-white/40"
          />
          <button
            type="submit"
